...
export function validateCreate(payload) {
  ...
  if (!isNonEmptyString(filename, 3) || !hasFileExtension(filename))
    errors.push("filename must be provided and contain an extension");
...
export function validatePut(payload) {
  ...
  if (filename !== undefined && (!isNonEmptyString(filename, 3) || !hasFileExtension(filename)))
    errors.push("filename must be provided and contain an extension");
  if (contentType !== undefined && !isAllowedContentType(contentType))
    errors.push(`contentType must be one of: ${ENUMS.CONTENT_TYPES.join(", ")}`);
...
/**
 * Updated:
 * - If the doc is NOT already REJECTED (logical delete) => reason is required
 * - If the doc is already REJECTED (physical delete) => reason is optional
 */
export function validateDelete(reason, isAlreadyRejected = false) {
